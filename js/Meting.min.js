	...
	loadMeting = function() {
		function a(a, t) {
			...
			if (t.length) {
				...
				for (var s in e) {
					var n = s.toLowerCase();
					(a.dataset.hasOwnProperty(n) || a.dataset.hasOwnProperty(s) || null !== e[s]) && (r[s] = a.dataset[
						n] || a.dataset[s] || e[s], "true" !== r[s] && "false" !== r[s] || (r[s] = "true" == r[s]))
				}
		...
		for (var e = 0; e < aplayers.length; e++)
			if (!aplayers[e].container.classList.contains("no-destroy")) try {
				aplayers[e].destroy()
			} catch (a) {
				console.log(a)
			}
		...
		for (var r = document.querySelectorAll(".aplayer"), s = 0; s < r.length; s++) {
			(function() {
				...
				if (e.classList.contains("no-destroy") && e.classList.add("no-reload"), e.dataset.id) {
					...
					n = n.replace(":server", e.dataset.server), n = n.replace(":type", e.dataset.type), n = n
						.replace(":id", e.dataset.id), n = n.replace(":auth", e.dataset.auth), n = n.replace(":r",
							Math.random());
